...
#define ENTITY_COUNT 1 // 적대적인 객체의 수 
...
typedef enum { MAINSTORY, GAMEOVER, GAMEFINAL } gameStory;
...
// 이혜원 코드 
int titleMain(); // 게임 시작 전 메인화면을 띄워주는 함수 
int keyControl();
void init(char*);
void titleDraw(); 
int menuDraw();
...
startProgram: // 프로그램 종료 메시지에서 아니오를 누르면 다시 돌아오기 위한 레이블 
	...
	for(;SystemCODE != PROGRAMEND;) { // 만약 플래그가 프로그램을 끝내는 방향이 아니면 
		titleMain(); // 게임 시작 전 메인화면을 계속 보여준다. 
	}
...
_mainstory:
	for(sceneNum=0;;sceneNum++) {
		...
		loadTEXTData(sceneList[sceneNum], 10);
...
void printText(char* text, int time) {
	int i;
